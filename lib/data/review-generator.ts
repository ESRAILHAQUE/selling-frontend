...
function generateReviewText(
  productTitle: string,
  templateGroup: (typeof reviewTemplates)[0]
): string {
  const template =
  ...
  const productPlural = productName.endsWith("s")
    ? productName
    : productName + "s";
...
// Generate unique reviews for a product - Optimized for performance
export function generateProductReviews(
  ...
  // Limit reviews to max 5 to prevent slow generation
  const reviewCount = Math.min(count, 5);

  // Pre-shuffle names array using seeded random for consistency
  const shuffledNames: string[] = [];
  const usedIndices = new Set<number>();

  // Get random unique indices efficiently
  for (let i = 0; i < reviewCount && i < usaNames.length; i++) {
    let index;
    let attempts = 0;
    // Maximum 10 attempts to find unused index (should always succeed)
    do {
      index = Math.floor(seededRandom(productSlug + i) * usaNames.length);
      attempts++;
    } while (usedIndices.has(index) && attempts < 10);

    if (attempts < 10) {
      usedIndices.add(index);
      shuffledNames.push(usaNames[index]);
    } else {
      // Fallback: just use next available name
      shuffledNames.push(usaNames[i % usaNames.length]);
    }
  }

  // Generate reviews from shuffled names
  for (let i = 0; i < shuffledNames.length; i++) {
    ...
    // Get appropriate template group (cache lookup instead of find)
    const templateGroup =
      rating === 5 ? reviewTemplates[0] : reviewTemplates[2];
